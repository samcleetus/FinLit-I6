...
func _ready() -> void:
	...
	var year_idx: int = state.current_year_index if state else -1
	var month_idx: int = state.current_month if state else -1
	print("Match: _ready -> session_mode=%s run_id=%s year=%s month=%s" % [_session_mode_to_string(mode), run_id, year_idx, month_idx])
	set_process(true)
